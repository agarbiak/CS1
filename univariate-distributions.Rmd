...
1. Define and explain the key characteristics of the discrete distributions: geometric, binomial, negative binomial, hypergeometric, Poisson and uniform on a finite set.
2. Define and explain the key characteristics of the continous distributions: normal, lognormal, exponential, gamma, chi-square, $t$, $F$, beta and uniform on an interval.
...
The geometric probability distribution ($X$) can be defined by the number of failures ($k$) in Bernoulli trials before achieving a success. A Bernoulli trial is a random experiment with exactly two outcomes, \{"_success_", "_failure_"\}, in which the probability of success is constant in every experiment. More formally, if we have $k \in \{0,\,1,\,2,\, \dots \}$ independent trials before a "_success_" with the $P($"_success_"$)$ on each trial defined as $p$, then:

$$P(X = k) = (1 - p)^{k}p$$

We have $X \sim Geo(p)$ with $p \in (0, 1]$.

In `R` we can generate random deviates following a geometric distribution using the function `rgeom(n = number of observations, prob = probability of success)`{.R} as follows:

```{r univariate-distributions-geometric-1}
# Guarantee reproducibility
set.seed(42)

# Generate 5 random deviates following Geo(0.25)
rgeom(5, 0.25)
```

If we wanted to find the 99<sup>th</sup> percentile of $X \sim Geo(p)$ we would use the quantile function, `qgeom(p = percentile of interest, prob = probability of success)`{.R}. It is also worth noting that `qgeom()`{.R} has an optional argument `lower.tail` taking Boolean inputs (_TRUE_, _FALSE_) with the default being _TRUE_. We can use this to find the top `p` percentile with `qgeom(p = percentile of interest, prob = probability of success, lower.tail = FALSE)`{.R}. Alternatively we can just plug in 1 - `p` with the default argument value.

```{r univariate-distributions-geometric-2}
# Find the 99th percentile of Geo(0.25)
...
If we wish to find $P(X \leq k)$ then we need to use the function for the cumulative distribution function, `pgeom(q = k failures before success, prob = probability of success)`{.R}.
...
The binomial probability distribution ($Y$) can be defined by the number of successes in a sequence of $n$ Bernoulli trials. A Bernoulli trial is a random experiment with exactly two outcomes, \{"_success_", "_failure_"\}, wherein the probability of success is $p$. We often state the probability of failure as $q = 1 - p$ for convenience. More formally, if we have $k \in \{0,\,1,\,2,\, \dots,\, n \}$ successes given $n$ independent trials with the $P($"_success_"$)$ on each trial defined as $p$, then:
...
- _size_ is the number of independent trials performed, and
...
Find the 99<sup>th</sup> percentile

```{r univariate-distributions-negative-binomial-4}
# Find the 99th percentile of Z~NegBin()
```

Plot the distribution:

```{r univariate-distributions-negative-binomial-5}
# Plot the distribution function of Z~NegBin()
```
...
# Find the 99th percentile of X~Hypergeo()
```

```{r univariate-distributions-hypergeometric-5}
# Plot the distribution function of X~Hypergeo()
```
